...
const FEATURES = [
  { icon:'🤖', color:'rgba(245,158,11,0.15)', title:'AI Tutor, 24/7', desc:'Get instant help on any topic, personalised to your curriculum and learning style.' },
  { icon:'🌍', color:'rgba(56,189,248,0.12)', title:'Your curriculum, not ours', desc:'We auto-map to your country\'s education system the moment you sign up.' },
  { icon:'🏆', color:'rgba(16,185,129,0.12)', title:'Gamified motivation', desc:'XP, streaks, badges, and leaderboards keep you coming back every day.' },
];
...
export default function Login() {
  ...
  const demoLogin = async () => {
    setForm({ email: 'demo@learnverse.com', password: 'Demo1234' });
    setApiError('');
    setLoading(true);
    try {
      // Simulate successful login for demo
      await new Promise(r => setTimeout(r, 800));
      // Store fake token and user for demo
      localStorage.setItem('lv_token', 'demo_token_xyz');
      localStorage.setItem('lv_user', JSON.stringify({
        id: 1, email: 'demo@learnverse.com',
        first_name: 'Alex', last_name: 'Kimani',
        country: 'KE', level: 24, xp: 4820,
    ...
    <div className="auth-page">
      ...
      <div className="auth-left">
        ...
        <div className="auth-left-inner">
          ...
          <p className="auth-sub">120,000+ students across 50 countries trust LearnVerse to get better grades.</p>
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            {FEATURES.map((f,i) => (
              <div key={i} className="auth-left-feature">
                <div className="auth-left-feature-icon" style={{ background:f.color }}>{f.icon}</div>
                <div>
        ...
        <div className="auth-right-inner fade-up">
          ...
          <form className="auth-form" onSubmit={handleSubmit}>
            ...
            <div style={{ textAlign:'right', marginTop:-8 }}>
              <button type="button" className="auth-switch-link" onClick={() => toast.info('Reset password', 'Check your email for a reset link.')}>
                Forgot password?
              </button>
            </div>
          ...
          <Button variant="secondary" size="lg" fullWidth onClick={demoLogin} style={{ marginTop:8 }}>
            🎮 Try demo account
          </Button>
